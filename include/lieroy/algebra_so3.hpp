...
namespace lieroy {
template <class T>
Eigen::Matrix<T, 3, 3> skew_symetric_matrix(
    const Eigen::Matrix<T, 3, 1>& vector) {
    Eigen::Matrix<T, 3, 3> m;
    m << 0.0, -vector(2), vector(1), vector(2), 0.0, -vector(0), -vector(1),
        vector(0), 0.0;
...
template <class T>
AlgebraSO3<T>::AlgebraSO3(T w1, T w2, T w3) : vector() {
    vector(0, 0) = w1;
    vector(1, 0) = w2;
    vector(2, 0) = w3;
}
...
template <class T>
AlgebraSO3<T>::AlgebraSO3(const std::array<T, 3>& v) : vector() {
    for (int i = 0; i < 3; i++) {
        vector(i, 0) = v[i];
    }
}
...
template <class T>
AlgebraSO3<T>::AlgebraSO3(const Eigen::Matrix<T, 3, 1>& v) : vector(v) {}
...
SO3<T> AlgebraSO3<T>::exp() const {
    ...
    const Eigen::Matrix<T, 3, 3> result =
        cos_theta * Eigen::Matrix<T, 3, 3>::Identity() +
...
template <class T>
AlgebraSO3<T>& AlgebraSO3<T>::operator=(const AlgebraSO3<T>& other) {
    vector = other.vector;
}
}
#endif
