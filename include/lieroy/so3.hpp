...
template <class T>
SO3<T>::SO3(const Eigen::Matrix<T, 3, 3>& m) : matrix(m) {}

template <class T>
SO3<T> SO3<T>::identity() {
    return SO3(Eigen::Matrix<T, 3, 3>::Identity());
}
...
AlgebraSO3<T> SO3<T>::log() const {
    ...
    if (fabs(sinphi_ba) > 1e-9) {
        ...
        // ** Note with this method we do not know the sign of 'phi', however
        // since we know phi is
        //    close to pi or 2*pi, the sign is unimportant..

        // Find the eigenvalues and eigenvectors
        Eigen::SelfAdjointEigenSolver<Eigen::Matrix<T, 3, 3> > eigenSolver(
            matrix);
...
AlgebraSO3<T> SO3<T>::log_rodrigues() const {
    ...
    if (cos_theta > (T)0.999856) {
        ...
        factor = (T)1.0 + theta_2 * ((T)1 / 6.0) + theta_2 * ((T)3 / 40.0) +
                 theta_2 * ((T)5 / 112.0);
...
SO3<T> SO3<T>::from_rpy(T roll, T pitch, T yaw) {
    ...
    transform.rotate(
        Eigen::AngleAxis<T>(roll, Eigen::Vector3d::UnitX().cast<T>()));
    transform.rotate(
        Eigen::AngleAxis<T>(pitch, Eigen::Vector3d::UnitY().cast<T>()));
    transform.rotate(
        Eigen::AngleAxis<T>(yaw, Eigen::Vector3d::UnitZ().cast<T>()));
...
template <class T>
SO3<T>& SO3<T>::operator=(SO3<T>&& rhs) {
    matrix = rhs.matrix;
    return *this;
}
