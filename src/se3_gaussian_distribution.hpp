...
template <typename T>
SE3GaussianDistribution<T>::SE3GaussianDistribution(const SE3<T>& mean,
                                                    const Eigen::Matrix<T, 6, 6>& covariance)
    : mean(mean), covariance(covariance) {}

template <typename T>
SE3GaussianDistribution<T>::SE3GaussianDistribution(const SE3GaussianDistribution<T>& other)
    : mean(other.mean), covariance(other.covariance) {}

  template <typename T>
  std::unique_ptr<SE3Distribution<T>> SE3GaussianDistribution<T>::copy() const {
    return std::unique_ptr<SE3GaussianDistribution<T>>(new SE3GaussianDistribution<T>(*this));
  }

template <typename T>
SE3<T> SE3GaussianDistribution<T>::sample() const {
  ...
  return perturbation_algebra.exp() * mean;
}
...
Eigen::Matrix<T, 6, 6> SE3GaussianDistribution<T>::covariance_rpy_to_lie(
  ...
  for (int i = 0; i < sample_size; i++) {
    ...
    SO3<double> rotation =
        SO3<double>::from_rpy(rpy_vector(3, 0), rpy_vector(4, 0), rpy_vector(5, 0));
